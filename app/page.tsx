...
export default function Home() {
  return (
    <>
      ...
      {/* ── 1. Hero (full-bleed photo + glass panel) ─────────────────────────── */}
      <section className="hero-glass">
        {/* Copy in a soft dark glass panel (top layer) */}
        <div className="hero-glass-inner">
          <div className="hero-panel">
            ...
            <p className="w-hero-sub" style={{ margin: '0 0 26px', maxWidth: 420 }}>
            ...
            <div className="hero-proof">
              {[
                ...
                <span key={t} className="hero-proof-item">
                  <span aria-hidden className="hero-proof-check">✓</span>
                  {t}
                </span>
              ))}
            </div>
        ...
        {/* Full-bleed cinematic photo background */}
        ...
        <style>{`
          ...
          .hero-panel {
            ...
            border-radius: 20px;
            background: linear-gradient(158deg, rgba(11,24,40,0.74), rgba(7,17,29,0.58));
            backdrop-filter: blur(16px) saturate(125%);
            -webkit-backdrop-filter: blur(16px) saturate(125%);
            border: 1px solid rgba(255,255,255,0.08);
            box-shadow: 0 40px 80px -34px rgba(0,0,0,0.7), inset 0 1px 0 rgba(255,255,255,0.06);
          }
          .hero-headline { text-wrap: balance; }

          .hero-proof { display: flex; gap: 18px; flex-wrap: wrap; align-items: center; margin-top: 2px; }
          .hero-proof-item {
            display: flex; align-items: center; gap: 9px;
            font: 500 13px/1 var(--font-text,sans-serif);
            color: var(--c-text-2,rgba(255,255,255,0.78));
          }
          .hero-proof-check {
            display: inline-flex; align-items: center; justify-content: center;
            width: 18px; height: 18px; border-radius: 999px; flex-shrink: 0;
            background: rgba(201,154,90,0.18);
          ...
          .hero-cta-row .w-cta-pill.outline:hover { transform: translateY(-2px); background: rgba(255,255,255,0.10); }

          @media (max-width: 900px) {
            .hero-glass { display: flex; flex-direction: column; min-height: 0; }
            .hero-glass-inner {
              position: static; min-height: 0; max-width: 100%;
              padding: 64px 22px 36px; justify-content: center;
            }
            .hero-panel {
              margin: 0 auto; max-width: 520px; text-align: center;
              /* No photo directly behind on mobile — make the panel solid + flat */
              background: transparent;
              backdrop-filter: none; -webkit-backdrop-filter: none;
              border: none; box-shadow: none; padding: 0;
            }
            .hero-panel .w-cta-row { justify-content: center !important; }
            .hero-proof { justify-content: center; }
            .hero-bg { position: static; width: 100%; aspect-ratio: 16 / 10; }
          }
