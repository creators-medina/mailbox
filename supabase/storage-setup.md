...
**`mail-envelopes` — SELECT (download)**
```sql
-- Customers can download envelope images for their own mail
((bucket_id = 'mail-envelopes') and (
  exists (
...
**`mail-envelopes` — INSERT/UPDATE/DELETE**
```sql
-- Admins only
(bucket_id = 'mail-envelopes') and public.is_admin()
```
...
```bash
supabase storage create mail-envelopes --public=false
